...
class FDParser : public BstIdl::FParser
{
public:
    // Run the parser on file F.  Return 0 on success.
    const std::list<std::shared_ptr<FDModel>> &parse(const std::string &filename);
    // Run the parser on the content string.
    static std::shared_ptr<FDModel> parseString(const std::string &content);
};
} // namespace BstIdl
