...
template <typename T> std::string initVarFObject(T *type)
{
    if (!type)
        return "";
    return getTypeName(type) + " $NAME = {0};\n";
...
static inline bool registerInitVar()
{
    ...
    t->forFArrayType = initVarFObject<FArrayType>;
    t->forFEnumerationType = initVarFObject<FEnumerationType>;
    t->forFStructType = initVarFObject<FStructType>;
